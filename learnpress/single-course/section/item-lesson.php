...
<li <?php learn_press_course_lesson_class( $item->ID ); ?> data-type="<?php echo $item->post_type;?>">
	...
	<<?php echo $tag; ?> class="course-item-title button-load-item" target="<?php echo $target; ?>" <?php echo $viewable ? 'href="' . $course->get_item_link( $item->ID ) . '"' : ''; ?> data-id="<?php echo $item->ID; ?>" data-complete-nonce="<?php echo wp_create_nonce( 'learn-press-complete-' . $item->post_type . '-' . $item->ID ); ?>">

		<?php echo '<i class="fa fa-bookmark-o lms-lesson-icon"></i> '. apply_filters( 'learn_press_section_item_title', get_the_title( $item->ID ), $item ); ?> 
