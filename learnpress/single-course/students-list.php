...
$students_list_heading = apply_filters('learn_press_students_list_heading', __('Students Enrolled', 'cryptic'));
...
<?php do_action('learn_press_before_student-list') ?>
    <div class="course-students-list">
        <?php if ($students_list_heading): ?>
            <h3 class="students-list-title"><?php echo wp_kses_post($students_list_heading); ?></h3>
        <?php endif; ?>
        ...
        <?php if ($students = $course->get_students_list(true, $student_limit)): ?>
            <ul class="students">
                <?php foreach ($students as $student):
                    ...
                    <li>
                        ...
                        <a
                            ...
                            title="<?php echo wp_kses_post($student->display_name) . ' profile' ?>"
                        >
                            <?php echo wp_kses_post($student->display_name); ?>
                        </a>
                    </li>
                <?php endforeach; ?>
                <?php
                ...
                if ($other_student) {
                    echo '<p>and ' . sprintf(_n('one student enrolled.', '%s students enrolled.', $other_student, 'cryptic'), $other_student) . '</p>';
                }
                ?>
            </ul>
        <?php else: ?>
            <div class="students empty">
                <?php if ($course->students) {
                    echo apply_filters('learn_press_course_count_student', sprintf(_n('One student enrolled.', '%s students enrolled.', $course->students, 'cryptic'), $course->students));
                } else {
                    echo apply_filters('learn_press_course_no_student', __('No student enrolled.', 'cryptic'));
                } ?>
            </div>
        <?php endif; ?>
    </div>
